...
<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex">
            @if ($todo->completed)
             <input wire:click="toggle({{ $todo->id }})" type="checkbox"  class="mr-3" checked>
            @else
             <input wire:click="toggle({{ $todo->id }})" type="checkbox"  class="mr-3" >
            @endif
    ...
    <span class="text-xs text-gray-500"> {{ $todo->created_at->todatestring()}} </span>
    <div class="mt-3 text-xs text-gray-700">
        @if ($editingTodoId === $todo->id)
                ...
                <button wire:click="cancalEdit" class="mt-3 px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600">Cancel</button>
       @endif
    </div>
</div>
